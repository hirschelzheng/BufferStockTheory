...
\begin{defn}
Suppose $\mathscr{A}$ and $\mathscr{B}$ are subsets of $\mathbb{R}$. Let $\mathcal{C}(\mathscr{A},\mathscr{B})$ be the space of continuous functions from $\mathscr{A}$ to $
\mathscr{B}$, and $\phiFunc \in \mathcal{C}(\mathscr{A},\mathscr{B})$ such that $\phiFunc >0$. A function $\bullet\in \mathcal{C}(\mathscr{A},\mathscr{B})$ is said to be $\phiFunc$-bounded if the $\phiFunc$-norm of $\bullet$,
\begin{equation}
